(function($){
	$.fn.getTwitterFeed = function(userid, count, reply){
		...
		if ($(window).width() > 600){
			var url = 'https://api.x.com/1/statuses/user_timeline.json?screen_name='+userid+'&count='+count+'&include_rts=true&trim_user=true&exclude_repies='+(reply ? '0' : '1')+'&callback=?';
			banner.show();
